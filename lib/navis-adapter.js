...
module.exports = CoreObject.extend({
  ...
  upload: function(buffer) {
     ...
     * tagging revisions.
     */
    var key = this.taggingAdapter.createTag().split(":")[1];
  ...
  _printRevisions: function(revisions) {
    ...
    var revisionsList = this._revisionsList(revisions);
    var formattedRevisionList = revisionsList.reduce(function(prev, current, index) {
      return prev + '\n\n' + (index + 1) + ') ' + current.replace(this.indexPrefix, '');
    }.bind(this), '');

    var footer = green('\n\nUse the following command to activate a revision:\n\n`ember deploy:activate --revision <VERSION>`\n');
    var message = header + formattedRevisionList + footer;
